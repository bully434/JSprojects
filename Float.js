...
function splitString(number, separator) {
  var array = number.split(separator);
  return array;
}
array=splitString(number, '.');
...
function dectohex(secondpart) {
	...
	for (var i=0; i < parseInt(mantissaLeng)+10; i++)
	{
		...
		if (secondpart >= 1){
			result=result+secondpart.toString().charAt(0);
			secondpart-=1;
		}
		else result+="0";
	}  
...
splittedHex=splitString(transformationResult,'.');
...
console.log();
return exp=new Array(transformationResult,orderResult,readyResult);
}
...
if (zero) sumOrder = (orderLeng/2)-1;
